...
#ifndef PAGE_NTPCONFIGURATION_H
#define PAGE_NTPCONFIGURATION_H
...
<tr><td align="right">Manual time:</td><td><input type="text" id="hours" name="hours" size="2" value="00">:<input type="text" id="minutes" name="minutes" size="2" value="00"></td></tr>
...
void send_NTP_configuration_html()
{
	if (server.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < server.args(); i++ ) {
			if (server.argName(i) == "ntpserver") PKConfig.setNTPServerName(urldecode(server.arg(i)));
			if (server.argName(i) == "update") PKConfig.setNTPUpdateInterval(server.arg(i).toInt());
			if (server.argName(i) == "tz") PKConfig.setTimeZone(server.arg(i).toInt());
			if (server.argName(i) == "hours") ActualTime.setHour(server.arg(i).toInt());
			if (server.argName(i) == "minutes") ActualTime.setMinute(server.arg(i).toInt());
			if (server.argName(i) == "dst") daylight = true;
		}
		PKConfig.setDaylightSavingMode(daylight);
		ActualTime.CreateUnixTimeStamp();
...
{
	String values ="";
	values += "ntpserver|" + (String) PKConfig.getNTPServerName() + "|input\n";
	values += "update|" +  (String) PKConfig.getNTPUpdateInterval() + "|input\n";
	values += "tz|" +  (String) PKConfig.getTimeZone() + "|input\n";
	values += "hours|" + (String) ActualTime.getHour() + "|input\n";
	values += "minutes|" + (String) ActualTime.getMinute() + "|input\n";
	values += "dst|" +  (String) (PKConfig.isDaylightSavingMode() ? "checked" : "") + "|chk\n";
