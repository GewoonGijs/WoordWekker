...
void send_network_configuration_html()
{
	...
	if (server.args() > 0 )  // Save Settings
	{
		...
		for ( uint8_t i = 0; i < server.args(); i++ ) {
			if (server.argName(i) == "ssid") PKConfig.setSSID( urldecode(server.arg(i)) );
			if (server.argName(i) == "password") PKConfig.setPassword( urldecode(server.arg(i)));

			if (server.argName(i) == "ip_0") if (checkRange(server.arg(i))) 	IP[0] =  server.arg(i).toInt();
			if (server.argName(i) == "ip_1") if (checkRange(server.arg(i))) 	IP[1] =  server.arg(i).toInt();
			if (server.argName(i) == "ip_2") if (checkRange(server.arg(i))) 	IP[2] =  server.arg(i).toInt();
			if (server.argName(i) == "ip_3") if (checkRange(server.arg(i))) 	IP[3] =  server.arg(i).toInt();

			if (server.argName(i) == "nm_0") if (checkRange(server.arg(i))) 	nm[0] =  server.arg(i).toInt();
			if (server.argName(i) == "nm_1") if (checkRange(server.arg(i))) 	nm[1] =  server.arg(i).toInt();
			if (server.argName(i) == "nm_2") if (checkRange(server.arg(i))) 	nm[2] =  server.arg(i).toInt();
			if (server.argName(i) == "nm_3") if (checkRange(server.arg(i))) 	nm[3] =  server.arg(i).toInt();

			if (server.argName(i) == "gw_0") if (checkRange(server.arg(i))) 	gw[0] =  server.arg(i).toInt();
			if (server.argName(i) == "gw_1") if (checkRange(server.arg(i))) 	gw[1] =  server.arg(i).toInt();
			if (server.argName(i) == "gw_2") if (checkRange(server.arg(i))) 	gw[2] =  server.arg(i).toInt();
			if (server.argName(i) == "gw_3") if (checkRange(server.arg(i))) 	gw[3] =  server.arg(i).toInt();

			if (server.argName(i) == "dhcp") dhcp = true;
		}
		PKConfig.setDHCP(dhcp);
		server.send_P ( 200, "text/html", PAGE_WaitAndReload );
		PKConfig.setIP(IPAddress(IP[0], IP[1], IP[2], IP[3]));
    PKConfig.setNetmask(IPAddress(nm[0], nm[1], nm[2], nm[3]));
		PKConfig.setGateway(IPAddress (gw[0], gw[1], gw[2], gw[3]));


//		Serial.print("IP Address:");
//	  Serial.println(PKConfig.getIP());
...
void send_network_configuration_values_html()
{
	...
	values += "ssid|" + PKConfig.getSSID() + "|input\n";
	values += "password|" +  PKConfig.getPassword() + "|input\n";

	IP = PKConfig.getIP();
	values += "ip_0|" +  (String) IP[0] + "|input\n";
	values += "ip_1|" +  (String) IP[1] + "|input\n";
	values += "ip_2|" +  (String) IP[2] + "|input\n";
	values += "ip_3|" +  (String) IP[3] + "|input\n";

	IP = PKConfig.getNetmask();
	values += "nm_0|" +  (String) IP[0] + "|input\n";
	values += "nm_1|" +  (String) IP[1] + "|input\n";
	values += "nm_2|" +  (String) IP[2] + "|input\n";
	values += "nm_3|" +  (String) IP[3] + "|input\n";

	IP = PKConfig.getGateway();
	values += "gw_0|" +  (String) IP[0] + "|input\n";
	values += "gw_1|" +  (String) IP[1] + "|input\n";
	values += "gw_2|" +  (String) IP[2] + "|input\n";
	values += "gw_3|" +  (String) IP[3] + "|input\n";
	values += "dhcp|" +  (String) (PKConfig.getDHCP() ? "checked" : "") + "|chk\n";
